defmodule Adyen.Recurring.V67.RecurringDetailsResult do
  @moduledoc """
  Provides struct and type for a RecurringDetailsResult
  """

  @type t :: %__MODULE__{
          creationDate: DateTime.t() | nil,
          details: [Adyen.Recurring.V67.RecurringDetailWrapper.t()] | nil,
          lastKnownShopperEmail: String.t() | nil,
          shopperReference: String.t() | nil
        }

  (
    @derive Jason.Encoder
    defstruct [:creationDate, :details, :lastKnownShopperEmail, :shopperReference]
  )

  @doc false
  @spec __fields__(atom) :: keyword
  def __fields__(type \\ :t)

  def __fields__(:t) do
    [
      creationDate: {:string, "date-time"},
      details: [{Adyen.Recurring.V67.RecurringDetailWrapper, :t}],
      lastKnownShopperEmail: :string,
      shopperReference: :string
    ]
  end
end
